...
namespace Nest
{
	internal class StoredFilterQueryJsonConverter : JsonConverter
	{
		...
		{
			var t = value as IStoredFilterQuery;
			if (t == null) return;
		...
		{
			var filter = new StoredFilterQueryDescriptor<object>();
			IStoredFilterQuery f = filter;
		...
		private void ReadTerms(IStoredFilterQuery StoredFilterQuery, JsonReader reader, JsonSerializer serializer)
		{
			reader.Read();
			if (reader.TokenType == JsonToken.StartObject)
			{
				...
				StoredFilterQuery.FilterLookup = ef;
			}
		}
	}
