...
namespace Nest
{
	public static class Query<T> where T : class
	{
		...
		public static QueryContainer StoredFilter(Func<StoredFilterQueryDescriptor<T>, IStoredFilterQuery> selector) =>
			new QueryContainerDescriptor<T>().StoredFilter(selector);
